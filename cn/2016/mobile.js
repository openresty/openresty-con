...
function renderLecturerAtMobile() {
	...
	about.innerHTML = __LECTURER_ABOUT__.map(function(l) {
...
function renderScheduleAtMobile() {
	...
	// })
	document.getElementById('schedule-list').innerHTML = __SCHEDULE__.map(function(s, i) {
		return '<li>' + scheduleTmpl.replace(/{(\w+)}/g, function($1, $2) {
			return s[$2] ? s[$2] : ''
		}) + '</li>'
	}).join(' ')
